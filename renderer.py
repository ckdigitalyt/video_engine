...
def render_timeline(json_path, output_path):
    ...
    for audio in timeline['audio_timeline']:
        clip = AudioFileClip(audio['file'])
        clip = clip.set_start(audio['start_time'])
        audio_clips.append(clip)

    video_clips = []
    for video in timeline['video_timeline']:
        clip = VideoFileClip(video['file'])
        clip = clip.set_start(video['start_time']).set_end(video['end_time'])
        video_clips.append(clip)

    final_audio = CompositeAudioClip(audio_clips)
    final_video = CompositeVideoClip(video_clips, size=tuple(timeline['render_settings']['resolution']))
    final_video = final_video.set_audio(final_audio)

    print(f"Rendering {output_path} on CPU... This will take time.")
