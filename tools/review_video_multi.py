#!/usr/bin/env python3
"""
review_video_multi.py — Cross-provider video review (Grok + ZAI GLM).
...
ZAI GLM review: glm-5.3-flash receives a "visual transcript"
(per-frame descriptions generated by Grok) + script + shot timeline, and
produces the same structured review schema.

Usage:
    python3 tools/review_video_multi.py --video results/.../mixed.mp4 \
        --frames tmp/review_frames --script script_final.json \
        --timeline timeline.json --outdir results/... --providers grok,zai
"""
...
load_dotenv(override=True)  # .env holds the real keys (e.g. ZAI_API_KEY)
...
def _zai_glm(prompt_text, max_tokens=4000):
    import urllib.request
    ...
    for attempt in range(5):
        try:
            ...
            msg = str(e)
            print(f"  !! zai attempt {attempt+1} failed: {msg[:150]}")
            if "429" in msg or "5" in msg[:3] or "timeout" in msg.lower():
                time.sleep(20 * (attempt + 1))
                continue
            raise
    raise RuntimeError("zai: all attempts failed")
...
def main():
    ...
    ap.add_argument("--providers", default="grok,zai")
    args = ap.parse_args()
    ...
    if "grok" in args.providers:
        ...
        # Frame descriptions for ZAI GLM (visual transcript)
        ...
        try:
            ...
            # v19n.3: grok desc failure must not kill the batch — ZAI GLM
            # can still run with a pre-existing frame_descriptions.json
            # (describe_frames.py) or an empty transcript.
            print(f"  !! frame descriptions failed ({str(e)[:100]}) — ZAI GLM may fall back to existing transcript")

    if "zai" in args.providers:
        print(f"=== ZAI GLM REVIEW ({os.environ.get('ZAI_MODEL', 'glm-5.3-flash')}) ===")
        ...
        out = _zai_glm(prompt)
        review = _parse_json(out)
        review["_meta"] = {"provider": "zai", "model": os.environ.get("ZAI_MODEL", "glm-5.3-flash"), "elapsed_s": round(time.time() - t0, 1)}
        with open(os.path.join(args.outdir, "review_zai_glm.json"), "w") as f:
            json.dump(review, f, indent=2)
        results["zai"] = review
        print(f"  score {review.get('quality_score')}/100 (conf {review.get('confidence')})")
