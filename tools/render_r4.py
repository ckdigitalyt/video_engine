#!/usr/bin/env python3
"""render_r4.py — dino_v2 r4 render driver (V4 phase 4, hero i2v pass).

Owned by the 2026-08-31 ~11:30 UTC ZeroGPU-reset agent. Re-renders exactly
the 6 pending hero shots (S01, S03, S05, S09, S11, S18) through the real
AI_VIDEO path: broker.image_to_video(hero_keyframe, manifest prompt,
manifest duration, manifest seed) — the manifest note in
results/dino_v2/hero_keyframes/manifest.json is the spec of record. Every
other shot reuses its r3 render record/clip untouched. Assembles
master_r4.mp4 and runs the full §19 publish gate (same gates as r3).

Scope guards:
  * Provider order per manifest: hf_router_i2v first — BUT the live test
    (scripts/live_aivideo_test.py, 2026-08-31 phase) re-confirmed HTTP 402
    QUOTA_EXHAUSTED on the hf_router monthly-credit wallet, so this driver
    removes hf_router from the broker instead of burning a known-dead
    wallet on every call (directive §7/§8 failure semantics).
  * ZeroGPU budget is account-level and small (~120s/day equivalent):
    on QUOTA_EXHAUSTED the driver stops the hero pass gracefully and the
    remaining hero shots keep their r3 fallback records (partial
    completion is acceptable per the task brief).
  * r3 artifacts (master_r3.mp4, publish_gate_r3.json, shots/r3/...) are
    never written.

Usage: venv/bin/python tools/render_r4.py [--no-vision] [--skip-qa]
"""
...
logger = logging.getLogger("render_r4")

OUT_DIR = PROJECT_ROOT / "results" / "dino_v2"
R4_SHOTS_DIR = OUT_DIR / "shots" / "r4"
MANIFEST_PATH = OUT_DIR / "hero_keyframes" / "manifest.json"
FPS = 30
WIDTH, HEIGHT = 1920, 1080
HERO_SHOTS = ("S01", "S03", "S05", "S09", "S11", "S18")
...
def main(argv: list[str] | None = None) -> int:
    ...
    r3_records = json.loads((OUT_DIR / "render_records_r3.json").read_text())

    script_doc, shots, edit_log = apply_edits(script_doc, shotlist["shots"])
    (OUT_DIR / "edit_log_r4.json").write_text(json.dumps(
    ...
    # ── records: non-hero shots reuse r3 verbatim ──
    records_path = OUT_DIR / "render_records_r4.json"
    records: dict[str, dict] = {}
    for shot in shots:
        sid = shot["shot_id"]
        if sid in HERO_SHOTS:
            continue
        base = r3_records.get(sid)
        if base and base.get("ok") and base.get("path") \
                and Path(base["path"]).exists():
            rec = copy.deepcopy(base)
            rec.setdefault("metadata", {})["reused_from"] = "r3"
            records[sid] = rec
        else:  # never invent: re-render offline like r3 did
            from engine.v3.render.runner import render_shot

            rec = render_shot(shot, style, R4_SHOTS_DIR, fps=FPS,
                              resolution=(WIDTH, HEIGHT), aspect="16:9",
                              attempt=0,
                              available=lambda rid: rid != "AI_VIDEO")
            records[sid] = rec
        ok = "OK " if records[sid].get("ok") else "FAIL"
        logger.info("  %s %s via %s (reused r3)", sid, ok,
                    records[sid].get("renderer_used"))
    records_path.write_text(json.dumps(records, indent=2, default=str))

    # ── hero pass: broker i2v per manifest, exactly the 6 pending shots ──
    from engine.broker.broker import MediaBroker
    from engine.renderers.media.conform import conform_clip

    broker = MediaBroker()
    # §7/§8: hf_router monthly credits re-confirmed QUOTA_EXHAUSTED (HTTP 402)
    # by today's live test — remove the dead wallet instead of retrying it.
    ...
    for shot in shots:
        ...
        if entry is None:
            logger.error("%s: no manifest entry — keeping r3 record", sid)
            records[sid] = copy.deepcopy(r3_records[sid])
            continue
            ...
            rec["attempts"].append(
                {"renderer": "AI_VIDEO", "ok": False,
                 "error": "ZeroGPU account quota exhausted earlier in this "
                          "pass — hero pass stopped gracefully (§8); r3 "
                          "fallback kept"})
            records[sid] = rec
            logger.warning("  %s skipped — ZeroGPU quota exhausted "
                           "(graceful stop, r3 fallback kept)", sid)
            continue
        t_gen = time.time()
        try:
            ...
            out_path = R4_SHOTS_DIR / f"{sid}_aivideo.mp4"
            ...
            if cls == "QUOTA_EXHAUSTED":
                ...
                logger.warning("ZeroGPU quota exhausted — stopping hero pass "
                               ...
                               "their r3 fallback renders")
            # Graceful degradation: keep the r3 fallback clip so the master
            ...
            if base and base.get("ok") and base.get("path") \
                ...
                logger.info("  %s keeps r3 fallback clip (%s)", sid,
    ...
    master_r4 = OUT_DIR / "master_r4.mp4"
    master_new = OUT_DIR / "master.mp4"
    if master_new.exists() and master_new != master_r4:
        master_new.replace(master_r4)
    logger.info("assembled %s (%s KB, %.1fs)", master_r4.name,
                master_r4.stat().st_size // 1024,
                assembled.get("video_duration") or 0)

    # align plan durations to the final timeline (same as run.py/r3)
    ...
    gate = publish_gate(
        master_r4, shots, script_doc, research, style, reports,
        OUT_DIR / "publish_gate_r4.json",
